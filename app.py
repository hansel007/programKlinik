...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root@localhost/dbprogramklinik'
...
class Pendaftaran(db.Model):
    ...
    rm = db.Column(db.String(50), nullable=False)
    nama = db.Column(db.String(150), nullable=False)
    ...
    def __init__(self,rm,nama,tl,tg_lahir,jk,profesi,alamat,hp,keterangan):
        self.rm = rm
        self.nama = nama
        self.tl = tl
...
class Pasien(db.Model):
    ...
    def __init__(self,rm,nama,tg_lahir,jk,keluhan,diagnosa,resep,user_id,pendaftaran_id,tanggal):
        self.rm = rm
        self.nama = nama
        self.tg_lahir = tg_lahir
        self.jk = jk
...
def tambahdaftar():
    if request.method == "POST":
        ...
        db.session.add(Pendaftaran(rm,nama,tl,tg_lahir,jk,profesi,alamat,hp,keterangan))
        db.session.commit()
...
def diagnosis(id):
    ...
    if request.method == "POST":
        ...
        db.session.add(data)
        db.session.commit()
        db.session.add(Pasien(rm,nama,tg_lahir,jk,keluhan,diagnosa,resep,user_id,pendaftaran_id,tanggal))
        db.session.commit()
        return redirect(request.referrer)
